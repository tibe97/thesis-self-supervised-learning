""" 
    ...
    With additional momentum encoder
"""
...
def _projection_head(num_ftrs: int,
                    h_dims: int,
                    out_dim: int,
...
class MyNet_Momentum(nn.Module, _MomentumEncoderMixin):
    ...
    def __init__(self,
        ...
        super(MyNet_Momentum, self).__init__()
        ...
        self.projection_head = \
            _projection_head(num_ftrs, proj_hidden_dim, out_dim, num_mlp_layers)
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            p1 = self.prediction_mlp(z1) # We keep the same prediciton MLP
            ...
            # don't update features layer with SwAV Loss
            #z1 = z1.clone().detach()
            q1 = self.prototypes_layer(z1) # Also add momentum to prototypes?
