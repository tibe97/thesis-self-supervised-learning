...
class NNMemoryBankModule(MemoryBankModule):
    ...
    """
    def __init__(self, size: int = 2 ** 16, epsilon: float = 0.05):
    ...
    def sample_negatives(self,
        ...
        cluster_scores = torch.cat((torch.mm(bank_normed.t(), model.prototypes_layer.weight.t()), cluster_scores_positives))
        ...
        q_negatives = q[cluster_scores_positives.shape[0]+1:]
        ...
        #index_nearest_neighbours = torch.argmax(similarity_matrix, dim=1)
        #negatives = torch.index_select(bank, dim=0, index=index_nearest_neighbours)
        ...
        for i in range(similarity_matrix.shape[0]): # for each positive sample
            ...
            mask_indices = (row == p_cluster).flatten()
            for idx in mask_indices:
                row[idx] = -1
            index_nearest_neighbours = torch.topk(row, num_nn, dim=1).indices # take indices of top K most similar
            negatives = torch.index_select(bank, dim=0, index=index_nearest_neighbours)
