        ...
        for i in range(similarity_matrix_pos.shape[0]): # for each positive sample
            ...
            if not self.false_neg_remove:
                ...
                if num_false_negatives > 0:
                    idx_positives = torch.Tensor(list(set(torch.range(0, output.shape[0]-1)) - set(mask_positives))).to(torch.int).to(output.device) # removes indexes of false negatives 
                    ipdb.set_trace()
